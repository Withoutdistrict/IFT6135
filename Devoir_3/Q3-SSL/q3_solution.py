...
class SimSiam(nn.Module):
    ...
    def loss(self, p1, p2, z1, z2, similarity_function='CosineSimilarity'):
        ...
        if similarity_function == 'CosineSimilarity':
            cos_s = CosineSimilarity()
        else:
            return Exception

        return -1 / 2 * (cos_s(p1, z2).mean() + cos_s(p2, z1).mean())
...
class CosineSimilarity(Module):
    ...
    def forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        """
        norm_x1 = torch.norm(x1, p=2, dim=self.dim)
        norm_x2 = torch.norm(x2, p=2, dim=self.dim)

        dl_e = torch.ones(size=norm_x1.shape, device=norm_x1.device) * self.eps
        return ((x1 * x2).sum(self.dim) / torch.maximum(norm_x1 * norm_x2, dl_e)).squeeze()
